// uses boost::strict_lock instead of boost::lock_guard
...
void store_lines_from_robotstxt(const std::string &host)
{
	...
	{
		// boost::strict_lock calls lock() on the mutex in the constructor
		// and calls unlock() on the mutex in the destructor; always locks
		// the mutex in the constructor (no exception possible as is with
		// boost::lock_guard)
		boost::strict_lock<boost::mutex> lg(lines_mutex);
